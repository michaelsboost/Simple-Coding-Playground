...
// To enable a plugin pass set `paletteHints` option on jsEditor to
// `true`.
(function() {
  ...
  function findMarks(jsEditor, range) {
    var markers = [];
    var from = range.from;
    var to = range.to;
    ...
    while (lineNumber <= lastLine) {
      var line = jsEditor.getLineHandle(lineNumber);
  ...
  function updatePaletteWidgets(jsEditor, range) {
    var doc = jsEditor.getDoc();
    findMarks(jsEditor, range).filter(isPaletteMark).forEach(clear);

    var isFirstLine = true;
    jsEditor.eachLine(range.from.line, range.to.line + 1, function(line) {
      var text = line.text;
      var match = null;
      var offset = 0;
  ...
  function batchUpdate(jsEditor, change) {
    ...
    if (change.origin === "paste") {
      var done = jsEditor.getHistory().done;
      change = done[done.length - 1].changes[0]
    }

    while (change) {
      updatePaletteWidgets(jsEditor, change);
      change = change.next;
    }
  }

  CodeMirror.defineOption("paletteHints", false, function(jsEditor, current, past) {
    if (current) {
      jsEditor.on("change", batchUpdate);
      updatePaletteWidgets(jsEditor, {
        from: {line: jsEditor.firstLine(), ch: 0},
        to: {line: jsEditor.lastLine() + 1, ch: 0}
      });
    }
    else {
      jsEditor.off("change", batchUpdate);
      jsEditor.getAllMarks().filter(isPaletteMark).forEach(clear);
    }
  });
})();
