...
// To enable a plugin pass set `paletteHints` option on cssEditor to
// `true`.
(function() {
  ...
  function findMarks(cssEditor, range) {
    var markers = [];
    var from = range.from;
    var to = range.to;
    ...
    while (lineNumber <= lastLine) {
      var line = cssEditor.getLineHandle(lineNumber);
  ...
  function updatePaletteWidgets(cssEditor, range) {
    var doc = cssEditor.getDoc();
    findMarks(cssEditor, range).filter(isPaletteMark).forEach(clear);

    var isFirstLine = true;
    cssEditor.eachLine(range.from.line, range.to.line + 1, function(line) {
      var text = line.text;
      var match = null;
      var offset = 0;
  ...
  function batchUpdate(cssEditor, change) {
    ...
    if (change.origin === "paste") {
      var done = cssEditor.getHistory().done;
      change = done[done.length - 1].changes[0]
    }

    while (change) {
      updatePaletteWidgets(cssEditor, change);
      change = change.next;
    }
  }

  CodeMirror.defineOption("paletteHints", false, function(cssEditor, current, past) {
    if (current) {
      cssEditor.on("change", batchUpdate);
      updatePaletteWidgets(cssEditor, {
        from: {line: cssEditor.firstLine(), ch: 0},
        to: {line: cssEditor.lastLine() + 1, ch: 0}
      });
    }
    else {
      cssEditor.off("change", batchUpdate);
      cssEditor.getAllMarks().filter(isPaletteMark).forEach(clear);
    }
  });
})();
