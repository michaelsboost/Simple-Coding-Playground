...
// To enable a plugin pass set `paletteHints` option on mdEditor to
// `true`.
(function() {
  ...
  function findMarks(mdEditor, range) {
    var markers = [];
    var from = range.from;
    var to = range.to;
    ...
    while (lineNumber <= lastLine) {
      var line = mdEditor.getLineHandle(lineNumber);
  ...
  function updatePaletteWidgets(mdEditor, range) {
    var doc = mdEditor.getDoc();
    findMarks(mdEditor, range).filter(isPaletteMark).forEach(clear);

    var isFirstLine = true;
    mdEditor.eachLine(range.from.line, range.to.line + 1, function(line) {
      var text = line.text;
      var match = null;
      var offset = 0;
  ...
  function batchUpdate(mdEditor, change) {
    ...
    if (change.origin === "paste") {
      var done = mdEditor.getHistory().done;
      change = done[done.length - 1].changes[0]
    }

    while (change) {
      updatePaletteWidgets(mdEditor, change);
      change = change.next;
    }
  }

  CodeMirror.defineOption("paletteHints", false, function(mdEditor, current, past) {
    if (current) {
      mdEditor.on("change", batchUpdate);
      updatePaletteWidgets(mdEditor, {
        from: {line: mdEditor.firstLine(), ch: 0},
        to: {line: mdEditor.lastLine() + 1, ch: 0}
      });
    }
    else {
      mdEditor.off("change", batchUpdate);
      mdEditor.getAllMarks().filter(isPaletteMark).forEach(clear);
    }
  });
})();
